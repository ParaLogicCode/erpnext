...
def get_columns():
	...
	return [
		...
		{"fieldname": "legend", "label": _("Legend"), "fieldtype": "Data", "width": 450},
		{
			"fieldname": "amount",
			"label": _("Amount (AED)"),
			"fieldtype": "Currency",
...
def set_vat_accounts(filters):
	if not filters.get("company"):
		frappe.throw(_("Please select Company"))

	vat_accounts = frappe.db.get_value("Company", filters.get("company"),
		["sales_tax_account", "sales_tax_on_purchase_account"])
	vat_accounts = [d for d in vat_accounts if d]
	if not vat_accounts:
		frappe.throw(_("Please configure VAT Accounts for Company {0}").format(filters.get("company")))

	filters["vat_accounts"] = vat_accounts
