frappe.listview_settings['Vehicle'] = {
	...
	get_indicator: (doc) => {
		if (doc.delivery_document_type) {
			...
			return [__("Inactive"), "grey", "warehouse,is,not set|delivery_document_type,is,not set"];
		} else {
			return [__("Active"), "blue", "warehouse,is,set|delivery_document_type,is,not set"];
		}
	},
	onload: function(listview) {
		listview.page.fields_dict.variant_of.get_query = () => {
			return erpnext.queries.item({"is_vehicle": 1, "is_model": 1, "include_disabled": 1});
		}

		listview.page.fields_dict.item_code.get_query = () => {
			var variant_of = listview.page.fields_dict.variant_of.get_value('variant_of');
			var filters = {"is_vehicle": 1, "include_disabled": 1, "is_variant": 1};
			if (variant_of) {
